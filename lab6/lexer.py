...
class Lexer:
    ...
    def lex(self) -> list[Token]:
        ...
        while self.cursor < len(self.source):
            ...
            # BooleanToken
            token = self._lexBooleanToken()
            if token:
                tokens.append(token)
                continue
            
            # StringToken
            token = self._lexStringToken()
            if token:
                tokens.append(token)
                continue
    ...
    def _lexStringToken(self) -> Token | None:
        ...
        if ident[0] in ["\""] and ident[-1] in ["\""]:
            return Token(TokenKind.stringToken, ident[1:-1])

        self.cursor = origCurs
        return None

    def _lexBooleanToken(self) -> Token | None:
        origCurs = self.cursor
        ident = self._findIdent()

        if ident in ["t", "true"]:
            return Token(TokenKind.trueToken, "True")
        if ident in ["f", "false"]:
            return Token(TokenKind.falseToken, "False")
